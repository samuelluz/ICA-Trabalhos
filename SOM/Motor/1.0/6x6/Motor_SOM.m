...
%------KNN------------------------------
[labelM Ec] = KNN([dados labelDados],[dTeste labelTeste],Mmin,K);
%----------------------------------------- 
...
fprintf(fid,'Erro KNN: %f%%\n',Ec);
fprintf(fid,'*(Neuronio sobre Neuronio)\n*M =\n');
for a = 1:nN
    fprintf(fid,'%f\t',Mmin(a,:));
    fprintf(fid,'\n');
end

labelM = reshape(labelM,[side side]);
fprintf(fid,'Label Matricial M =\n');
for a = 1:side
